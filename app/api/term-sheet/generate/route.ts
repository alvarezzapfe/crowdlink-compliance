...
async function generateDocx(data: any, schedule: any[]) {
  ...
  const summaryRows = summaryItems.map(([label, value]) => new TableRow({ children: [new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: label, bold: true, size: 17, color: GRAY, font: 'Calibri' })] })], width: { size: 3654, type: WidthType.DXA }, shading: { type: ShadingType.SOLID, color: 'F9FAFB', fill: 'F9FAFB' }, margins: { top: 80, bottom: 80, left: 120, right: 80 } }), new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: value, size: 17, font: 'Calibri', color: '111827' })] })], width: { size: 6786, type: WidthType.DXA }, margins: { top: 80, bottom: 80, left: 120, right: 80 } })] }))
  const doc = new DocxDocument({ sections: [{ properties: { page: { margin: { top: 720, bottom: 720, left: 900, right: 900 } } }, children: [new Paragraph({ children: [new ImageRun({ data: logoBuffer, transformation: { width: 140, height: 35 }, type: 'png' })], spacing: { after: 200 } }), new Paragraph({ border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: PURPLE } }, spacing: { after: 300 }, children: [] }), new Paragraph({ children: [new TextRun({ text: 'TERM SHEET - PROPUESTA DE CREDITO', bold: true, size: 28, color: PURPLE, font: 'Calibri' })], spacing: { after: 80 } }), new Paragraph({ children: [new TextRun({ text: `Fecha de emision: ${new Date().toLocaleDateString('es-MX')}`, size: 18, color: GRAY, font: 'Calibri' })], spacing: { after: 400 } }), new Paragraph({ children: [new TextRun({ text: 'RESUMEN EJECUTIVO', bold: true, size: 20, color: PURPLE, font: 'Calibri' })], spacing: { after: 160 } }), new Table({ width: { size: 10440, type: WidthType.DXA }, rows: summaryRows, borders }), new Paragraph({ children: [], spacing: { after: 400 } }), new Paragraph({ children: [new TextRun({ text: 'TABLA DE AMORTIZACION', bold: true, size: 20, color: PURPLE, font: 'Calibri' })], spacing: { after: 160 } }), new Table({ width: { size: 10440, type: WidthType.DXA }, borders, rows: [new TableRow({ tableHeader: true, children: ['#', 'Fecha', 'Saldo Ini.', 'Interes', 'Amort.', 'Comis.', 'Total', 'Saldo Fin.'].map(hdrCell) }), ...schedule.map((r: any) => new TableRow({ children: [dataCell(String(r.periodo)), dataCell(r.fecha), dataCell(fmt(r.saldoInicial), true), dataCell(fmt(r.interes), true), dataCell(fmt(r.amortizacion), true), dataCell(r.comisiones > 0 ? fmt(r.comisiones) : '-', true, false, r.comisiones > 0 ? 'B45309' : '9CA3AF'), dataCell(fmt(r.total), true, true), dataCell(fmt(r.saldoFinal), true)] })), new TableRow({ children: [totCell('TOTALES'), totCell(''), totCell(''), totCell(fmt(totalInt), true), totCell(fmt(totalAmort), true), totCell(fmt(totalComis), true), totCell(fmt(totalPago), true), totCell('-', true)] })] }), new Paragraph({ children: [], spacing: { after: 400 } }), new Paragraph({ border: { top: { style: BorderStyle.SINGLE, size: 4, color: 'E5E7EB' } }, children: [new TextRun({ text: 'PorCuanto S.A. de C.V. - IFC - CNBV - Art. 47 CUITF - LFPDPPP - Documento confidencial.', size: 14, color: '9CA3AF', font: 'Calibri' })], spacing: { before: 200 } })] }] })
