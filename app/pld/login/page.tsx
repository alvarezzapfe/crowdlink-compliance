...
export default function PldLoginPage() {
  ...
  const handleCredentials = async () => {
    ...
    if (!token) { await supabase.auth.signOut(); setError('Error de sesión'); setLoading(false); return }
  ...
  const handleTotp = async () => {
    ...
    if (!res.ok) {
      ...
      setError(d.error || 'Código incorrecto'); setTotpCode(''); setLoading(false); return
    }
        ...
        <div style={{ width: '100%', maxWidth: '420px' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <h1 style={{ color: cl.gray900, fontSize: '1.3rem', fontWeight: '800', margin: '0 0 0.3rem', letterSpacing: '-0.02em' }}>
              {step === 'credentials' ? 'Sistema PLD' : 'Verificación 2FA'}
            </h1>
            <p style={{ color: cl.gray400, fontSize: '0.85rem', margin: 0 }}>
              {step === 'credentials' ? 'Prevención de Lavado de Dinero · Equipo Compliance' : 'Ingresa el código de Google Authenticator'}
            </p>
          </div>
          ...
          <div style={{ background: cl.white, border: `1px solid ${cl.gray200}`, borderRadius: '18px', padding: '2rem', boxShadow: '0 4px 16px rgba(0,0,0,0.05)' }}>
            ...
            {step === 'credentials' && (
              <div style={{ display: 'grid', gap: '1rem' }}>
                ...
                {error && <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '8px', padding: '0.7rem 0.9rem', color: '#DC2626', fontSize: '0.8rem' }}>{error}</div>}
            ...
            {step === 'totp' && (
              <div style={{ display: 'grid', gap: '1rem' }}>
                <div>
                  ...
                  <input
                    type="text" inputMode="numeric" value={totpCode} autoFocus
                    onChange={e => setTotpCode(e.target.value.replace(/\D/g, '').slice(0, 6))}
                    onKeyDown={e => e.key === 'Enter' && handleTotp()}
                    placeholder="0 0 0 0 0 0"
                    style={{ ...inp, textAlign: 'center', letterSpacing: '0.5em', fontSize: '1.4rem', fontWeight: '700' }}
                  />
                </div>
                {error && <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '8px', padding: '0.7rem 0.9rem', color: '#DC2626', fontSize: '0.8rem' }}>{error}</div>}
                <button onClick={handleTotp} disabled={loading || totpCode.length !== 6} style={{ width: '100%', background: loading || totpCode.length !== 6 ? '#E2E8F0' : '#059669', border: 'none', borderRadius: '10px', padding: '0.85rem', color: loading || totpCode.length !== 6 ? '#94A3B8' : 'white', cursor: loading || totpCode.length !== 6 ? 'not-allowed' : 'pointer', fontFamily: cl.fontFamily, fontSize: '0.9rem', fontWeight: '700' }}>
                  {loading ? 'Verificando...' : 'Acceder →'}
                </button>
                <button onClick={() => { setStep('credentials'); setTotpCode(''); setError('') }} style={{ background: 'none', border: 'none', color: cl.gray400, fontSize: '0.8rem', cursor: 'pointer', fontFamily: cl.fontFamily }}>
                  ← Regresar
                </button>
              </div>
            )}
